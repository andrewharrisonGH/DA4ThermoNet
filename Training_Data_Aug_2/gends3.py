...
# ... (generate_tasks function remains the same) ...
...
def main():
    ...
    # C, X, Y, Z are the dimensions of the features BEFORE transposition
    C = X = Y = Z = None 
    ...
    for task in probe_generator:
        try:
            ...
            if result is not None:
                ...
                # We need the dimensions for the output shape
                break
    ...
    num_mutations = sum(1 for line in open(args.input)) - 1 # Subtract 1 for the header
    ...
    # NOTE: D, H, W, C are the dimensions (16, 16, 16, 14) from the probe
    ...
    mmap.flush() 
    # The file might have empty rows at the end if some tasks failed.
    # The number of lines in the .txt file will be the true count of successful tasks.
